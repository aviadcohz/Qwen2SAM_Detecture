...
% The style file automatically anonymises authors and adds line numbers.
% After acceptance, switch to \usepackage[main, final]{neurips_2026}
% For arXiv preprint before / after the decision, use \usepackage[preprint]{neurips_2026}
...
\title{Qwen2SAM-Detecture: End-to-End VLM-Guided Multi-Texture Segmentation via Native-Space Bridging and Multiplexed Grounding}

% Anonymised for double-blind review. The ``default'' style option hides this block.
\author{%
  Anonymous Authors
}
...
We present \textbf{Qwen2SAM-Detecture}, an end-to-end architecture that realizes this brain-eyes coupling. Our central question is: \textbf{Can a VLM-guided system autonomously identify, describe, and segment multiple texture regions simultaneously, producing a consistent image partition in one forward pass?}
...
\begin{enumerate}
    ...
    \item \textbf{Decoupling Multi-Query Grounding: Block-Diagonal Attention + Batch Multiplexing.} We resolve two independent failure modes of parallel \texttt{[SEG]}-token grounding. A custom block-diagonal attention mask prevents \emph{Context Leakage} between sequential texture descriptions (reducing inter-\texttt{[SEG]} cosine similarity from 0.74 to 0.16). A batch-multiplexing reshape of the query tensor ($\mathbb{R}^{B \times K \times D} \to \mathbb{R}^{(BK) \times 1 \times D}$) forces each texture query into its own independent ``Slot 1'' forward pass, bypassing the 90.5\,\% / 0.0\,\% pretrained positional allocation bias of SAM~3's multi-prompt decoder.

    \item \textbf{Shifted-Zero LM-Loss Cliff and Masked-Row Trainable Grounding.} We identify the fundamental \emph{Count Collapse vs.\ Language Collapse} dilemma in multi-modal LoRA training and a subsequent \emph{Emission Failure} pathology that emerges once the dilemma is na\"ively resolved. Our solution has two components. First, a \emph{Shifted-Zero cliff} places the zero-weight slot one token \emph{past} \texttt{[SEG]} rather than at \texttt{[SEG]} itself---preserving geometric freedom for the grounding embedding while restoring the emission-learning gradient that a na\"ive $\lambda_{\text{SEG}} = 0$ cliff silently destroys under causal-shift semantics. Second, \emph{masked-row training} with \emph{warm-start from \texttt{<|im\_end|>}} unfreezes exactly the SEG rows of \texttt{embed\_tokens} and \texttt{lm\_head} (16{,}384 effective parameters) via a gradient-mask hook, calibrating \texttt{lm\_head[SEG]} so the token can actually be chosen at inference. Together they raise honest live RWTD mIoU from 0.000 to 0.794 at epoch 16.
...
The resulting architecture (Figure~\ref{fig:architecture}) comprises four components: a frozen Qwen3-VL-8B backbone with lightweight LoRA adapters and masked-row trainable SEG rows, the \textbf{Bridge} ($\mathrm{Linear}(4096, 1024) + \mathrm{LN} + \mathrm{GELU} + \mathrm{Dropout}(0.4)$, $\sim 4.2$M trainable parameters) composed with SAM~3's frozen pretrained resizer, a \emph{permanently frozen} SAM~3 mask decoder (no LoRA), and a Winner-Takes-All pixel assignment layer. Training employs a two-stage curriculum with the Shifted-Zero LM-loss cliff and ${\sim}8.2$M trainable parameters total. The design addresses five pathologies that we characterize in the following subsections.
...
% ----------------------------------------------------------------------------
\subsection{Freeing the Grounding Token: Shifted-Zero Cliff and Trainable Emission Rows}
...
This reveals a fundamental dilemma: \emph{binary} loss masking creates a binary failure mode. Full text loss $\to$ Count Collapse; zero text loss $\to$ Language Collapse.

\paragraph{An Intermediate Cosine-Decayed Variant.}
An earlier iteration smoothed the binary mask into a per-block cosine decay from $\lambda_{\max} = 1.0$ at the block start down to $\lambda_{\min} = 0.05$ at the \texttt{[SEG]} token. This prevented catastrophic forgetting (live-inference RWTD mIoU recovered from 0.136 to 0.694) but introduced a \emph{continuous} tug-of-war: with non-zero linguistic pressure on nearly every token in each block, the Dice mask loss could not drive the grounding embedding aggressively enough toward geometric optima. Empirically, training Dice plateaued at 0.43 and honest (exactly-K) RWTD mIoU stalled at 0.694.

\paragraph{Exponential Cliff: the Obvious-But-Broken Version.}
The apparent fix is to replace the gentle cosine with a sharp exponential cliff centred on the \texttt{[SEG]} token. Let $d(i)$ be the token-wise distance from position $i$ to the nearest assistant-region \texttt{[SEG]}; the ``obvious'' cliff is $\lambda(\text{SEG}) = 0$ and $\lambda(i) = 1 - e^{-\alpha d(i)}$ elsewhere, with $\alpha = 2.0$ yielding $\lambda = 0.865$ at distance one, $0.982$ at distance two, $\approx 1.0$ at distance $\geq 3$. This removes the continuous tug-of-war---gradients from the Dice path and the LM path only coincide at the geometrically-isolated \texttt{[SEG]} token---and recovers the teacher-forced (Oracle) regime: mIoU on RWTD under teacher-forced \texttt{[SEG]} embeddings climbs to 0.738 at epoch 12. Under \emph{live} (free-generation) inference, however, this design fails completely: the model never emits \texttt{[SEG]} at all, substituting \texttt{<|tool\_call|>} or \texttt{<|im\_end|>} in its place, and downstream RWTD mIoU collapses to 0.000.

\paragraph{Diagnosis: the Emission-Zero Conflation.}
Causal-LM loss at position $i$ supervises the \emph{prediction of token $i{+}1$}. The ``obvious'' cliff sets $\lambda(\text{SEG}) = 0$ in order to free $\mathbf{h}_{\text{SEG}}$ geometrically---but this same weight also controls the gradient that trains $\mathbf{h}_{\text{SEG}}$ to predict whatever comes \emph{after} \texttt{[SEG]} (typically a newline or the next \texttt{TEXTURE\_k:} marker). Zeroing it removes the ``continue correctly after a SEG emission'' signal, and with it the principal pathway that would otherwise teach the model to emit \texttt{[SEG]} as the natural prelude to the next region. Free-generation therefore substitutes pretrained-high-probability special tokens that Qwen already associates with ``end of a thought block''.

\paragraph{Our Solution (Part A): Shifted-Zero Cliff.}
We shift the zero one position forward:
\begin{equation}
    \lambda_{\text{LM}}(i) = \begin{cases}
        0 & \text{if } i - 1 \in \mathcal{S}_{\text{SEG,asst}} \\
        1 - e^{-\alpha \cdot d(i)} & \text{otherwise}
    \end{cases}, \qquad \alpha = 2.0.
\end{equation}
That is, the position \emph{after} each assistant-region \texttt{[SEG]} carries weight zero (a geometrically unused continuation slot), while the \texttt{[SEG]} position itself carries weight one. Under causal-shift semantics, $\lambda(\text{SEG}) = 1$ restores the emission-learning gradient on the post-SEG continuation; this gradient is predominantly syntactic (newlines, region markers) rather than semantic, so the Dice path and the LM path still do not conflict at the grounding embedding itself. The weighted LM loss is:
\begin{equation}
    \mathcal{L}_{\text{LM}}^{\text{SZ}} = \frac{\sum_{i \in \mathcal{A}} \lambda_{\text{LM}}(i) \cdot \ell_{\text{CE}}(i)}{\sum_{i \in \mathcal{A}} \lambda_{\text{LM}}(i)}
\end{equation}
where $\mathcal{A}$ denotes the set of assistant (non-prefix) token positions.

\paragraph{Our Solution (Part B): Masked-Row Trainable SEG with Warm-Start.}
Shifted-Zero alone remains insufficient. Even with the corrected weighting, live-inference RWTD mIoU stays at 0.000 while the Oracle climbs. Probing \texttt{lm\_head} reveals a second failure: the output-projection row \texttt{lm\_head.weight[SEG]}---the row whose inner product with the final hidden state produces \texttt{[SEG]}'s logit---is frozen at its random initialisation, and LoRA on \texttt{q\_proj}/\texttt{v\_proj} cannot reach it. With an uncalibrated \texttt{lm\_head[SEG]}, the SEG logit is dominated by logits from well-trained rows like \texttt{<|tool\_call|>} or \texttt{<|im\_end|>}, and these tokens are chosen instead.

We resolve this with two complementary mechanisms:
\begin{enumerate}
    \item \textbf{Warm-start from \texttt{<|im\_end|>}.} At model initialisation we copy the pretrained rows \texttt{lm\_head.weight[<|im\_end|>]} and \texttt{embed\_tokens.weight[<|im\_end|>]} into the corresponding SEG rows. This places \texttt{[SEG]} near a token Qwen already associates with ``finalise a region of thought'' in both the input and output embedding spaces---a semantically appropriate anchor that avoids the random-init cliff.
    \item \textbf{Masked-row gradient routing.} During Stage 2, we unfreeze the full \texttt{embed\_tokens.weight} and \texttt{lm\_head.weight} tensors (so PyTorch computes gradients for them at all) and register a gradient hook that zeros every row except the SEG row. This restricts actual weight updates to the two 8192-dimensional SEG rows---16{,}384 effective parameters---while leaving the remaining ${\sim}150$K rows of each tensor exactly as pretrained. The SEG rows are added to the optimiser in their own parameter group at a dedicated learning rate.
\end{enumerate}

Together, Shifted-Zero and Masked-Row cure emission: live-inference RWTD mIoU jumps from 0.000 to 0.794 at epoch 16 (Section~\ref{sec:experiments}), while Oracle mIoU continues its trajectory from 0.738 to 0.781 at the same epoch---confirming that the live gap was a pure emission problem, not a geometric one.
...
\paragraph{Stage 1: Bridge Warmup (Epochs 1--8).}
Only the Bridge, multi-texture mask head, and dustbin embedding are trainable (${\sim}4.4$M parameters). Qwen LoRA is frozen; the SEG embedding and \texttt{lm\_head} rows are frozen at their warm-started values (Section~\ref{sec:count_collapse}); SAM (including the reused resizer) is frozen. The Bridge absorbs the random-initialisation shock and learns the shape-matching $4096 \to 1024$ map into SAM's native text space under heavy Dropout(0.4) regularization before any linguistic gradients enter the system.

\paragraph{Stage 2: Qwen Sync + Bridge Decay + SEG Rows (Epochs 9--30).}
Three components activate simultaneously: (i) Qwen's LoRA adapters (rank 8, \texttt{q\_proj}+\texttt{v\_proj}; ${\sim}3.8$M parameters) are unfrozen at learning rate $1 \times 10^{-6}$ ($0.01\times$ base); (ii) the masked-row SEG training (Section~\ref{sec:count_collapse}) unlocks the SEG rows of \texttt{embed\_tokens} and \texttt{lm\_head} (16{,}384 effective parameters), enabling the model to learn to emit \texttt{[SEG]}; (iii) the Bridge's learning rate is \emph{decayed by $10\times$} (from $1 \times 10^{-4}$ to $1 \times 10^{-5}$). The Bridge decay is critical: maintaining the projector at full LR during co-training causes \emph{Latent Co-Adaptation Churn}---the projector's rapid weight updates continuously shift the feature distribution that the LoRA is trying to adapt to, preventing stable co-convergence. By slowing the Bridge once Qwen joins, we ensure the LoRA builds on a stable mapping rather than chasing a moving target. The Shifted-Zero cliff (Section~\ref{sec:count_collapse}) preserves linguistic coherence throughout.

There is no Stage~3. SAM~3 (including its resizer, cross-attention, and decoder) remains frozen across all 30 epochs.
...
\paragraph{Micro-Warmup Restart for Extension Runs.}
An initial 20-epoch schedule under a cosine LR curve yielded a peak at epoch 16 (live-inference honest RWTD mIoU = 0.794). To push further without a full LR re-warmup---which would shock weights already sitting in a good basin---we introduce a \emph{Micro-Warmup Restart} protocol. On resume from a late-stage checkpoint, we (a) rebuild the LR scheduler with each parameter group's peak set to \texttt{original\_base\_LR} $\times s$ (with $s = 0.15$ in our run), preserving per-group ratios between Bridge, Qwen LoRA, and SEG rows; (b) apply a short linear warmup (2 epochs) from zero to the scaled peak; (c) cosine-decay over the remaining epochs only. AdamW moment estimates are preserved across the restart, so the optimisation trajectory that produced the pre-restart optimum is retained; only the LR envelope changes.

% ============================================================================
% 4. DATA ECOSYSTEM
% ============================================================================
\section{Data Ecosystem: The Detecture Pipeline and Dual-Evaluation Strategy}
\label{sec:data}
...
\subsection{The Core Data Contribution: The Detecture Mining Pipeline}
\label{sec:pipeline}

We introduce the \textbf{Detecture Pipeline}, an automated, scalable framework designed to transform generic semantic datasets into high-quality texture-grounding data. The pipeline operates in three stages: intelligent filtering, geometric extraction, and VLM-based annotation.
...
\begin{itemize}
    \item \textbf{In-Domain Evaluation (ADE20K\_Detecture):} Verifies core architectural viability in a controlled setting where the visual distribution matches the training data. Stable metrics here confirm that architectural innovations do not introduce regressions.
...
We train on ${\sim}14{,}000$ Detecture-mined ADE20K samples with effective batch size 8 (batch~2, grad-accum~4) for a total of 30 epochs on a single GPU in bfloat16. The schedule is a two-stage curriculum (Section~\ref{sec:curriculum}): 8 epochs of Bridge warmup under frozen Qwen, followed by 22 epochs with Qwen LoRA, masked-row SEG training, and a $10\times$ Bridge LR decay jointly active. The 30-epoch horizon is composed of an initial 20-epoch cosine run followed by a 10-epoch Micro-Warmup Restart extension at $s = 0.15$ (Section~\ref{sec:curriculum}). All results reported below use the \emph{exactly-K} prompt to neutralise the Hungarian-matching inflation that the permissive ``1 to 6'' prompt induces.
...
+ Block-Diagonal Mask + \texttt{[SEG]} (V5 Oracle, teacher-forced) & 0.810 & Context Leakage \\
...
+ Bridge + Frozen Resizer + na\"ive Exponential Cliff $\lambda_{\text{SEG}} = 0$ (Oracle) & 0.738 & Bottleneck Semantic Squash \\
\quad same configuration, \emph{live} inference & 0.000 & --- (Emission Failure exposed) \\
+ Shifted-Zero Cliff (Oracle) & 0.781 & --- \\
+ Shifted-Zero + Masked-Row SEG + Warm-Start (live, ep 16) & \textbf{0.794} & Emission Failure \\
Zero-Shot SAM~3 + object-aware prompt (no training) & \textbf{0.928} & --- (upper bound) \\
\textbf{Our method} (30-ep schedule with Micro-Warmup Restart; live, best) & \emph{in progress} & --- \\
\bottomrule
\end{tabular}
\caption{Ablation on RWTD. Each fix addresses a distinct failure mode. Widening into SAM~3's native 1024-dim text space closes $+0.04$ on the teacher-forced Oracle. The na\"ive exponential cliff ($\lambda_{\text{SEG}} = 0$) inadvertently erases the emission-learning signal and drives live-inference mIoU to exactly $0.0$ despite Oracle continuing to rise---isolating the Emission Failure pathology invisible under teacher forcing. Shifting the zero one position forward and warm-starting the SEG rows from \texttt{<|im\_end|>} under masked-row routing closes the live gap to $0.794$ at epoch 16, \emph{surpassing} the best teacher-forced number from any prior bottleneck-family variant.}
\label{tab:ablation}
\end{table}

\paragraph{Live vs.\ Oracle at epoch 16.}
Notably, live-inference mIoU (0.794) slightly \emph{exceeds} teacher-forced Oracle mIoU (0.781) at the same checkpoint. We attribute this to distributional mismatch: Oracle feeds the Bridge with \texttt{[SEG]} hidden states produced under a fixed ``1 to 6'' prompt, whereas live inference uses the actual two-texture prompt the model was trained to handle. The gap disappears under a matched prompt.

\emph{Training is in progress under the Micro-Warmup Restart extension (epochs 21--30). Final quantitative results on RWTD and ADE20K\_Detecture, comparison with LISA and Sa2VA baselines, and qualitative visualisations will be reported upon convergence.}
...
We have presented Qwen2SAM-Detecture, an end-to-end architecture for parallel, multi-texture segmentation that bridges vision-language reasoning with geometric dense prediction. Through systematic ablation, we identified and resolved five fundamental pathologies---Context Leakage, Slot-1 Positional Bias, the Count Collapse / Language Collapse dilemma, Directional Drift, and Bottleneck Semantic Squash---plus a subtle \emph{Emission Failure} mode that emerges once the Count/Language dilemma is na\"ively resolved and which we cure with a Shifted-Zero cliff and masked-row trainable SEG rows. Our solutions---block-diagonal attention masking, batch multiplexing, the Shifted-Zero LM-loss cliff with masked-row training and warm-start from \texttt{<|im\_end|>}, and a Bridge-to-Native-Space projector that reuses SAM~3's own pretrained resizer---are general mechanisms applicable to any system coupling autoregressive VLM decoding with SAM-family decoders. The Zero-Shot SAM~3 experiment (0.928 RWTD mIoU with a hand-crafted object-aware prompt) is particularly instructive: it suggests that for any pretrained segmentation backbone, the correct role of a learned projector is \emph{not} to acquire a new semantic mapping but to preserve the one the backbone already carries. The Bridge + frozen-resizer design is a minimal realization of this principle.

\paragraph{Limitations.}
Several limitations bound the current work. \textbf{(1) Inference cost.} The two-pass protocol---autoregressive generation followed by block-diagonal extraction---is substantially slower than prompt-based segmentation; end-to-end latency is dominated by Qwen generation (${\approx}\,1$--$2$\,s per image on a single GPU) and scales with the number of predicted textures. \textbf{(2) SAM~3 as a hard ceiling.} Our frozen-SAM policy inherits SAM~3's semantic coverage: the 0.928 ZS upper bound on RWTD tightly upper-bounds the method on any domain where SAM natively understands descriptions, and leaves no recourse for domains where it does not. Unfreezing SAM to cross that ceiling consistently regressed out-of-domain generalisation in our ablations. \textbf{(3) Narrow benchmark.} RWTD contains 253 samples; honest exactly-$K$ numbers are stable but a richer multi-domain evaluation would strengthen the cross-domain claims. \textbf{(4) Hungarian-matching illusion.} The common ``1 to 6'' prompt inflates scores via Hungarian assignment over permissively-many predicted regions; we therefore report exactly-$K$ as the honest metric, but deployed systems receiving an unknown target count remain sensitive to prompt phrasing. \textbf{(5) Emission remains a learned behaviour.} Even with Shifted-Zero and masked-row training, \texttt{[SEG]} emission is not guaranteed under sufficiently out-of-distribution prompts; recovery requires either prompt normalisation or fallback regex extraction. \textbf{(6) Texture-specific framing.} We evaluate on texture segmentation; extending the method to open-vocabulary scene partitioning is plausible (below) but not empirically validated here.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TECHNICAL APPENDICES (do not count toward 9-page limit)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\appendix

\section{Detecture scoring formulas and thresholds}
\label{app:scoring}
The Detecture Pipeline (Section~\ref{sec:pipeline}) selects images via a composite score $s \in [0, 100]$ combining a mask-structure prior $s_A$ and a boundary-quality prior $s_{\text{geom}}$, weighted toward geometry: $s = 0.20 \cdot s_A + 0.60 \cdot s_{\text{geom}} + \text{bonuses} - \text{penalties}$. The mask-structure prior $s_A$ aggregates region count (2--4 preferred), size distribution (balanced vs.\ fragmented), and region entropy. The boundary-quality prior $s_{\text{geom}}$ aggregates gradient-transition strength along ground-truth boundaries, boundary coherence (fraction of pixels belonging to coherent edge segments), and area balance across regions. Bonuses are applied when ${\geq}65\%$ of pixels lie in texture-classified regions and when region count $\in \{2, 3, 4\}$. Penalties apply to object-dominated scenes (as determined by ADE20K class membership in our 32-term ``object'' list) and to scenes with more than 8 distinct regions. Images scoring below 65 are discarded; the final training set contains ${\sim}14{,}000$ images with an average of 3.2 regions each. The full semantic-keyword classification table (72 texture terms / 32 object terms / 10 ambiguous terms) is provided as a JSON file in the supplementary archive.

\section{VLM annotation prompt templates and validation}
\label{app:annotation}
For each filtered image, we extract up to 5 geometrically distinct texture regions by merging per-class ADE20K masks (each region $\geq 1\%$ image area). We then prompt Claude~3.5~Sonnet~\cite{claude} with both the original image and a colored-overlay visualisation showing the region boundaries, and ask for structured descriptions in the fixed template \texttt{"Texture of <semantic name>, <visual features>, <spatial context>"} at 10--15 words per region. The exact system prompt, user prompt, and few-shot examples used at annotation time are provided in the supplementary archive. Validation: we randomly sampled 100 images from the mined training set and manually verified that every emitted description (a) correctly names the material or surface type, (b) mentions at least one visual feature (colour, pattern, coarseness), and (c) anchors a spatial context (foreground / background / region). Pass rate was 96\%; the 4\% failures were all cases where the region was ambiguous (e.g., reflective glass) and the VLM produced a plausible but non-specific description rather than an incorrect one. For TextureSAM degree variants ($\eta > 0$), descriptions are reused from the $\eta = 0$ anchor since CNT augmentation preserves region geometry; this degree-expansion step yielded ${\sim}45\%$ additional samples at zero annotation cost.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% NEURIPS PAPER CHECKLIST (do not count toward 9-page limit)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
